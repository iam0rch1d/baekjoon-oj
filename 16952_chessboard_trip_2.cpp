    ...
    while (!bfsVertices.empty()) {
        ...
        REP(i, piece == 0 ? 8 : 4) {
            ...
            while (true) {
                ...
                if (chmin(nextState, state + make_pair(1, 0))) bfsVertices.emplace(ny, nx, nextNumber, piece);
        ...
        REP(i, 3) {
            ...
            if (chmin(dist[y][x][number][i], state + make_pair(1, 1))) bfsVertices.emplace(y, x, number, i);
        }
    }
